...
test.describe("Check your answers page", () => {
  ...
  test("renders expert details from session data", async ({ page }) => {
    ...
    await expect(page.getByText("Guideline rates or hours exceeded").first()).toBeVisible();
  });

  test("renders Expert details and Supporting documents card sections", async ({ page }) => {
    await expect(page.getByRole("heading", { name: "Expert details" })).toBeVisible();
    await expect(page.getByRole("heading", { name: "Expert costs" })).toHaveCount(0);
  ...
  test("submit sends the user to the application submitted page", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: "Prior authority application submitted" }),
    ).toBeVisible();
  });
});
